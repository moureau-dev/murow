...
> {
    ...
    build(): ComputeKernel<TBuffers> {
        ...
        attachShaderMetadata(this._shaderFn, () => {
            ...
            for (const name of Object.keys(bufferDefs)) {
                try {
                    // Typed layout accessor — works inside shader context (lazy call)
            ...
            // generated by tinyest for arithmetic on WGSL-typed values.
            externals.__tsover_mul = (std as any).mul;
            externals.__tsover_add = (std as any).add;
            externals.__tsover_sub = (std as any).sub;
            externals.__tsover_div = (std as any).div;
            return externals;
        }, true, { d, std });
